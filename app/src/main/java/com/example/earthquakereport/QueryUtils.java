...
*/
     ...
     */
//    public static ArrayList<EarthQuakeDetails> extractEarthquakes() {
//
...
//
//            /**Create a root json object by calling the constructor and passing the sample json response string. This will parse the whole
//            GL Json string we got from the USGS API*/
//            JSONObject baseJsonResponse=new JSONObject(SAMPLE_JSON_RESPONSE);
//            /**We need the magnitue, place and time . Therefore yo get that info, we need the features Array. Within the root
//             * JSONObject we want to get values related to the features key.
//             * It should map to a JSONArray that contains a list of earthquakes*/
//            JSONArray earthquakeArray=baseJsonResponse.getJSONArray("features");
//
//            //We want to loop through each earthquake in earthquake array
//            for(int i=0;i<earthquakeArray.length();i++)
//            {
//              //Pulling out the JSON object at the specified position of the earthquakeArray
//             JSONObject currentEarthquake=earthquakeArray.getJSONObject(i);
//             //Once we have the current object json object, we can extract ot the json object associated with the properties key.
//                JSONObject properties=currentEarthquake.getJSONObject("properties");
//                //String magnitude=properties.getString("mag");
//
//                //double value for using DecimalFormat to display magnitude till one decimal place
//                double magnitude=properties.getDouble("mag");
//                String place=properties.getString("place");
//                //String time= properties.getString("time");
//
//                //Extract the value for the key called "time" for correct format
//                long time=properties.getLong("time");
//
//                //Extract the value for the key called "url"
//                String url=properties.getString("url");
//
//                //Creating new earthquakedetails object from these strings
//                EarthQuakeDetails earthquake=new EarthQuakeDetails(magnitude,place,time,url);
//                //add this to earthquakes list (array)
...
//    }
    ...
    /**Modify the extractEarthquakes() method that handles JSON parsing. I renamed it to extractFeatureFromJson() and to take a JSON response String as input. Instead of
     * hardcoding the extractFeatureFromJson() method to only be able to parse the hardcoded SAMPLE_JSON_RESPONSE String, this method becomes more
     * reusable in different contexts if we accept a String input.*/
/**Return a list of {@link EarthQuakeDetails} objects that has been built up from
 * parsing the given JSON response
 */
        private static List<EarthQuakeDetails> extractFeatureFromJson(String earthquakeJson)
        {
            ...
            try {
                ...
                for (int i = 0; i < earthquakeArray.length(); i++) {
                    ...
                    //Extract the value for the key called "msg";
                    String location = properties.getString("place");

                    //Extract the value of key called time
                    long time=properties.getLong("time");
                    ...
                    //Create a new {@link EearthquakeDetails} object with the magnitude, locationtime,
                    //             and url from the JSON response.
                    EarthQuakeDetails earthquake = new EarthQuakeDetails(magnitude, location, time, url);
                    earthquakes.add(earthquake);
                }
            }
                catch(JSONException e)
            ...
            /**Add in the fetchEarthquakeData() helper method that
             * ties all the steps together - creating a URL, sending
             * the request, processing the response. Since this is
             * the only “public” QueryUtils method that the EarthquakeAsyncTask needs to interact with, make all other
             * helper methods in QueryUtils “private”.*/

public static List<EarthQuakeDetails> fetchEarthquakeData(String requestURL)
{
    try {
        Thread.sleep(2000);
    ...
    List<EarthQuakeDetails> earthquakes = extractFeatureFromJson(jsonResponse);
    ...
    private static String makeHttpRequest(URL url)throws IOException
    {
        ...
        try {
            ...
            if (urlConnection.getResponseCode() == 200) {
                ...
                Log.e(LOG_TAG, "PError response code." + urlConnection.getResponseCode());
            }
        }
        catch(IOException e)
        {
